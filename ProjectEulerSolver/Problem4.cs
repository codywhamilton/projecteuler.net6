...
namespace ProjectEulerSolver
{
    ...
    public class Problem4 : IProblem
    {
        ...
        public String GetSolution()
        {
            ...
            for (int i = _UpperBound; i >= _LowerBound; i--)
            {
                for (int j = _UpperBound; j >= _LowerBound; j--)
                {
                    ...
                    { // we have already found a larger palindrome for i*j short cercuite the inner loop.
                        j = _LowerBound;
                    }
                    else
                    {
        ...
        {
            return new ProblemDescription("Largest palindrome product", 4, "Find the largest palindrome made from the product of two 3-digit numbers.");
        }
    }
}