...
*/
    ...
    public class Problem11 : IProblem
    {
        readonly int[][] _Numbers = {
            ...
            new int[] { 6, 39, 05, 42, 96, 35, 31, 47, 55, 58, 88, 24, 00, 17, 54, 24, 36, 29, 85, 57 },
        ...
        const int _RunLength = 4;
        public string GetSolution()
        {
            int prodResult = 0, prodPossibleResult = 0;
            for (int i = 0; i < _Numbers.Count(); i++)
            {//[7][9]
                int[] numberLine = _Numbers[i];
                for (int j = 0; j < numberLine.Count(); j++)
                {
                    //if can check sideways while in int[][] do so
                    if (j + _RunLength < numberLine.Count())
                    {
                        prodPossibleResult = numberLine[j] * numberLine[j + 1] * numberLine[j + 2] * numberLine[j + 3];
                    }
                    if (prodPossibleResult > prodResult)
                    {
                        prodResult = prodPossibleResult;
                    }
                    //if can check down while in int[][] do so
                    if (i + _RunLength < _Numbers.Count())
                    {
                        prodPossibleResult = _Numbers[i][j] * _Numbers[i + 1][j] * _Numbers[i + 2][j] * _Numbers[i + 3][j];
                    }
                    if (prodPossibleResult > prodResult)
                    {
                        prodResult = prodPossibleResult;
                    }
                    //if can check diagonally left to right while in int[][] do so
                    if (i + _RunLength < _Numbers.Count() && j + _RunLength < numberLine.Count())
                    {
                        prodPossibleResult = _Numbers[i][j] * _Numbers[i + 1][j + 1] * _Numbers[i + 2][j + 2] * _Numbers[i + 3][j + 3];
                    }
                    if (prodPossibleResult > prodResult)
                    {
                        prodResult = prodPossibleResult;
                    }
                    //if can check diagonally right to left while in int[][] do so
                    if (i + _RunLength < _Numbers.Count() && j - _RunLength > 0)
                    {
                        prodPossibleResult = _Numbers[i][j] * _Numbers[i + 1][j - 1] * _Numbers[i + 2][j - 2] * _Numbers[i + 3][j - 3];
                    }
                    if (prodPossibleResult > prodResult)
